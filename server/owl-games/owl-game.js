...
const validateMatch = (_match) => {
  ...
  match.hasMatchData = _match.title !== '' && _match.video !== '' && _match.thumb !== '';
  // Are there at least 5 child games?
  match.hasGameData = _match.games.length >= 5;
...
const validateTeamString = (_team) => {
  ...
  if (_team === 'LDN') { team = 'LON'; }
  // CHD => CDH
  if (_team === 'CHD') { team = 'CDH'; }
  return team;
};
...
class SheetMatchData {
  constructor(_rawData) {
    ...
    const week = this.gameDate[1] < 0 ? 'Playoffs' : ` Week ${this.gameDate[1]}`;
    const swd = `Stage ${this.gameDate[0]}${week} Day ${this.gameDate[2]}`;
